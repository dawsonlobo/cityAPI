...
export async function addCities() {
  ...
  const existingCities = await City.find({ name: { $in: cities.map((city) => city.name) } });
  ...
  const newCities = cities.filter((city) => !existingCityNames.includes(city.name));
...
export async function addStates() {
  const states = [
    { name: 'California', population: 39538223, gdp: 3100000, capital: 'Sacramento' },
  ...
  const existingStates = await State.find({ name: { $in: states.map((state) => state.name) } });
  ...
  const newStates = states.filter((state) => !existingStateNames.includes(state.name));
